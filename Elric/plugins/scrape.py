...
from Elric import user
from Elric.events import bot_cmd
from telethon.utils import *
from telethon.tl.functions.messages import ImportChatInviteRequest

from Elric.core.tools import getcards
...
async def _(m):
    ...
    if 'joinchat' in channel:
        ...
        if all(ele is None for ele in resolve):
            ...
            if not chat_hash:
                return await m.reply("Invalid link.")
            try:
                chat_invite = await user(ImportChatInviteRequest(chat_hash[0]))
    ...
    all_cards = []
    # all_mess = await  user.get_messages(ent, limit = amount)
    async for event in user.iter_messages(channel, limit = amount, wait_time = 5 if amount > 1000 else 2):
        if not event.text:
            continue
...
<b>Host</b> -» <a href="https://t.me/Elric_xD">𝐁𝐥𝐚𝐳𝐞-𝐱𝐃</a>
"""
